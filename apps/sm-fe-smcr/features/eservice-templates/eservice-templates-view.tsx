"use client";

import {
  Check,
  ChevronDown,
  ChevronLeft,
  ChevronRight,
  Cpu,
  LayoutTemplate,
  Radio,
  Search,
  ShieldCheck,
  X,
} from "lucide-react";
import { ReactNode, useMemo, useState } from "react";

import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
} from "@/components/ui/popover";
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from "@/components/ui/select";
import type { EserviceTemplate } from "@/lib/services/eservice-templates.service";
import { cn } from "@/lib/utils";

import {
  createEserviceTemplateColumns,
  EserviceTemplatesTable,
  normalizeTemplateMode,
  normalizeTemplateTechnology,
  TEMPLATE_MODE_ORDER,
  TemplateModeBadge,
  TemplateTechnologyBadge,
} from "./table";

const fmtNum = new Intl.NumberFormat("it-IT");

type SortKey = "name" | "technology" | "mode";
type SortDir = "asc" | "desc";

type Props = {
  rows: EserviceTemplate[];
};

export function EserviceTemplatesView({ rows }: Props) {
  const [query, setQuery] = useState("");
  const [technology, setTechnology] = useState("");
  const [mode, setMode] = useState("");
  const [sort, setSort] = useState<{ key: SortKey; dir: SortDir }>({
    key: "name",
    dir: "asc",
  });
  const [pageSize, setPageSize] = useState(25);
  const [page, setPage] = useState(1);

  const total = rows.length;

  const allTechnologies = useMemo(() => {
    return [
      ...new Set(rows.map((r) => normalizeTemplateTechnology(r.technology))),
    ].sort((a, b) => a.localeCompare(b, "it"));
  }, [rows]);

  const allModes = useMemo(() => {
    const fromData = [
      ...new Set(rows.map((r) => normalizeTemplateMode(r.mode))),
    ];
    return fromData.sort((a, b) => {
      const ai = TEMPLATE_MODE_ORDER.indexOf(
        a as (typeof TEMPLATE_MODE_ORDER)[number],
      );
      const bi = TEMPLATE_MODE_ORDER.indexOf(
        b as (typeof TEMPLATE_MODE_ORDER)[number],
      );
      if (ai !== -1 && bi !== -1) return ai - bi;
      if (ai !== -1) return -1;
      if (bi !== -1) return 1;
      return a.localeCompare(b, "it");
    });
  }, [rows]);

  const personalDataCount = useMemo(
    () => rows.filter((r) => r.personalData).length,
    [rows],
  );

  const signalHubCount = useMemo(
    () => rows.filter((r) => r.isSignalHubEnabled).length,
    [rows],
  );

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    return rows.filter((r) => {
      if (
        q &&
        !r.name.toLowerCase().includes(q) &&
        !r.description.toLowerCase().includes(q)
      ) {
        return false;
      }
      if (
        technology &&
        normalizeTemplateTechnology(r.technology) !== technology
      ) {
        return false;
      }
      if (mode && normalizeTemplateMode(r.mode) !== mode) return false;
      return true;
    });
  }, [rows, query, technology, mode]);

  const sorted = useMemo(() => {
    const arr = [...filtered];
    const { key, dir } = sort;
    arr.sort((a, b) => {
      const av = a[key];
      const bv = b[key];
      return dir === "asc"
        ? av.localeCompare(bv, "it")
        : bv.localeCompare(av, "it");
    });
    return arr;
  }, [filtered, sort]);

  const totalPages = Math.max(1, Math.ceil(sorted.length / pageSize));
  const safePage = Math.min(page, totalPages);
  const start = (safePage - 1) * pageSize;
  const pageRows = sorted.slice(start, start + pageSize);

  const rangeLabel =
    sorted.length === 0
      ? "0 di 0"
      : `${start + 1} – ${Math.min(start + pageSize, sorted.length)} di ${sorted.length}`;

  const clearFilters = () => {
    setQuery("");
    setTechnology("");
    setMode("");
    setPage(1);
  };

  const hasFilters = query.trim() !== "" || technology !== "" || mode !== "";

  const columns = useMemo(
    () =>
      createEserviceTemplateColumns({
        sort,
        onSort: (key) => {
          if (sort.key === key) {
            setSort({ key, dir: sort.dir === "desc" ? "asc" : "desc" });
          } else {
            setSort({ key, dir: "asc" });
          }
        },
      }),
    [sort],
  );

  return (
    <div className="flex flex-col gap-6">
      <header className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold tracking-tight">
          Template e-service PDND
        </h1>
        <p className="text-muted-foreground max-w-3xl text-sm md:text-base">
          Template pubblicati sul catalogo PDND Interoperabilità.
        </p>
      </header>

      <section className="grid gap-4 md:grid-cols-4">
        <Card className="md:col-span-2 border shadow-sm gap-0 py-0 bg-gradient-to-br from-teal-50 to-white border-teal-100">
          <CardContent className="flex flex-col gap-2 p-5 md:p-6">
            <div className="text-muted-foreground flex items-center gap-2 text-sm font-medium">
              <span className="bg-teal-100 text-teal-700 inline-flex size-7 items-center justify-center rounded-md">
                <LayoutTemplate className="size-4" />
              </span>
              Totale template pubblicati
            </div>
            <p className="text-5xl font-extrabold tracking-tight tabular-nums">
              {fmtNum.format(total)}
            </p>
            <p className="text-muted-foreground text-xs">
              {filtered.length !== total
                ? `${fmtNum.format(filtered.length)} corrispondono ai filtri attivi`
                : "Tutti i template attualmente disponibili"}
            </p>
          </CardContent>
        </Card>

        <Card className="border shadow-sm gap-0 py-0">
          <CardContent className="flex flex-col gap-2 p-5">
            <div className="text-muted-foreground flex items-center gap-2 text-sm font-medium">
              <span className="bg-amber-100 text-amber-700 inline-flex size-7 items-center justify-center rounded-md">
                <ShieldCheck className="size-4" />
              </span>
              Con dati personali
            </div>
            <p className="text-2xl font-bold tracking-tight tabular-nums">
              {fmtNum.format(personalDataCount)}
            </p>
            <p className="text-muted-foreground text-xs">
              Template che trattano dati personali
            </p>
          </CardContent>
        </Card>

        <Card className="border shadow-sm gap-0 py-0">
          <CardContent className="flex flex-col gap-2 p-5">
            <div className="text-muted-foreground flex items-center gap-2 text-sm font-medium">
              <span className="bg-blue-100 text-blue-700 inline-flex size-7 items-center justify-center rounded-md">
                <Radio className="size-4" />
              </span>
              Signal Hub attivo
            </div>
            <p className="text-2xl font-bold tracking-tight tabular-nums">
              {fmtNum.format(signalHubCount)}
            </p>
            <p className="text-muted-foreground text-xs">
              Template abilitati alla notifica eventi
            </p>
          </CardContent>
        </Card>
      </section>

      <Card className="overflow-hidden border shadow-sm py-0 gap-0">
        <CardContent className="space-y-4 p-4 md:p-6">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="flex flex-wrap items-center gap-2">
              <div className="relative w-full md:w-72">
                <Search className="text-muted-foreground absolute left-3 top-1/2 size-4 -translate-y-1/2" />
                <Input
                  value={query}
                  onChange={(e) => {
                    setQuery(e.target.value);
                    setPage(1);
                  }}
                  placeholder="Cerca per nome o descrizione…"
                  className="pl-9 pr-9"
                  aria-label="Cerca per nome o descrizione"
                />
                {query && (
                  <Button
                    type="button"
                    variant="ghost"
                    onClick={() => setQuery("")}
                    className="text-muted-foreground hover:text-foreground absolute right-2 top-1/2 h-auto -translate-y-1/2 rounded p-1 hover:bg-transparent has-[>svg]:p-1"
                    aria-label="Pulisci ricerca"
                  >
                    <X className="size-3.5" />
                  </Button>
                )}
              </div>
              <OptionCombo
                value={technology}
                onChange={(v) => {
                  setTechnology(v);
                  setPage(1);
                }}
                options={allTechnologies}
                icon={<Cpu className="size-4 opacity-70" />}
                placeholder="Tutte le tecnologie"
                clearLabel="Rimuovi filtro tecnologia"
                renderOption={(opt) => (
                  <TemplateTechnologyBadge technology={opt} />
                )}
              />
              <OptionCombo
                value={mode}
                onChange={(v) => {
                  setMode(v);
                  setPage(1);
                }}
                options={allModes}
                icon={<LayoutTemplate className="size-4 opacity-70" />}
                placeholder="Tutte le modalità"
                clearLabel="Rimuovi filtro modalità"
                renderOption={(opt) => <TemplateModeBadge mode={opt} />}
              />
              {hasFilters && (
                <Button
                  type="button"
                  variant="ghost"
                  size="sm"
                  onClick={clearFilters}
                >
                  <X className="size-3.5" />
                  Azzera filtri
                </Button>
              )}
            </div>
            <div className="text-muted-foreground text-sm">
              <span className="text-foreground font-semibold tabular-nums">
                {fmtNum.format(filtered.length)}
              </span>{" "}
              di {fmtNum.format(total)} template
            </div>
          </div>

          <EserviceTemplatesTable
            columns={columns}
            data={pageRows}
            isEmpty={sorted.length === 0}
          />

          <div className="flex flex-col gap-3 border-t pt-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-muted-foreground flex items-center gap-2 text-sm">
              <Select
                value={String(pageSize)}
                onValueChange={(v) => {
                  setPageSize(Number(v));
                  setPage(1);
                }}
              >
                <SelectTrigger
                  className="h-8 w-[72px]"
                  aria-label="Righe per pagina"
                >
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="10">10</SelectItem>
                  <SelectItem value="25">25</SelectItem>
                  <SelectItem value="50">50</SelectItem>
                  <SelectItem value="100">100</SelectItem>
                </SelectContent>
              </Select>
              <span>per pagina</span>
            </div>
            <p className="text-muted-foreground text-center text-sm">
              {rangeLabel}
            </p>
            <div className="flex justify-center gap-1 sm:justify-end">
              <Button
                type="button"
                variant="outline"
                size="icon"
                className="size-8"
                disabled={safePage <= 1}
                onClick={() => setPage((p) => Math.max(1, p - 1))}
                aria-label="Pagina precedente"
              >
                <ChevronLeft className="size-4" />
              </Button>
              <Button
                type="button"
                variant="outline"
                size="icon"
                className="size-8"
                disabled={safePage >= totalPages}
                onClick={() => setPage((p) => Math.min(totalPages, p + 1))}
                aria-label="Pagina successiva"
              >
                <ChevronRight className="size-4" />
              </Button>
            </div>
          </div>
        </CardContent>
      </Card>
    </div>
  );
}

function OptionCombo({
  value,
  onChange,
  options,
  icon,
  placeholder,
  clearLabel,
  renderOption,
}: {
  value: string;
  onChange: (v: string) => void;
  options: string[];
  icon: ReactNode;
  placeholder: string;
  clearLabel: string;
  renderOption: (option: string) => ReactNode;
}) {
  const [open, setOpen] = useState(false);

  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          type="button"
          variant="outline"
          className="min-w-[200px] justify-between font-normal"
        >
          <span className="flex items-center gap-2 truncate">
            {icon}
            <span className="truncate">
              {value ? renderOption(value) : placeholder}
            </span>
          </span>
          <span className="ml-2 flex items-center gap-1">
            {value && (
              <span
                role="button"
                tabIndex={0}
                onClick={(e) => {
                  e.stopPropagation();
                  onChange("");
                }}
                onKeyDown={(e) => {
                  if (e.key === "Enter" || e.key === " ") {
                    e.preventDefault();
                    e.stopPropagation();
                    onChange("");
                  }
                }}
                className="text-muted-foreground hover:text-foreground inline-flex items-center"
                aria-label={clearLabel}
              >
                <X className="size-3.5" />
              </span>
            )}
            <ChevronDown className="size-4 opacity-70" />
          </span>
        </Button>
      </PopoverTrigger>
      <PopoverContent align="start" className="w-[280px] p-1">
        <div className="max-h-72 overflow-y-auto">
          <Button
            type="button"
            variant="ghost"
            onClick={() => {
              onChange("");
              setOpen(false);
            }}
            className={cn(
              "hover:bg-muted hover:text-foreground flex h-auto w-full items-center justify-between rounded-md px-2 py-1.5 text-left text-sm font-normal",
              value === "" &&
                "bg-teal-50 text-teal-800 font-medium hover:text-teal-800",
            )}
          >
            <span>{placeholder}</span>
            {value === "" && <Check className="size-4" />}
          </Button>
          {options.map((opt) => (
            <Button
              key={opt}
              type="button"
              variant="ghost"
              onClick={() => {
                onChange(opt);
                setOpen(false);
              }}
              className={cn(
                "hover:bg-muted hover:text-foreground flex h-auto w-full items-center justify-between rounded-md px-2 py-1.5 text-left text-sm font-normal",
                value === opt &&
                  "bg-teal-50 text-teal-800 font-medium hover:text-teal-800",
              )}
            >
              {renderOption(opt)}
              {value === opt && <Check className="size-4 shrink-0" />}
            </Button>
          ))}
        </div>
      </PopoverContent>
    </Popover>
  );
}
